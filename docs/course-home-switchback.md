...
- 中英双语切换，浏览器会记住上次选择。
- 8 个商品 SKU 展示，商品图已下载到本地仓库，不依赖外部图片链接。
...
```text
assets/meilong-products/
```

当前页面不再热链外部图片，商品卡片、商品详情、产业源头和合规区块都引用仓库内图片。这样 GitHub + EdgeOne 部署后，图片会随站点一起发布，避免外部图床访问失败导致裂图。

当前图片为可免费使用的真实珠宝/首饰摄影素材，用作业务样站展示。后续如果拿到梅陇本地工厂实拍图，只需要保持文件名不变替换同名 JPG，网站内容和功能不用改。
