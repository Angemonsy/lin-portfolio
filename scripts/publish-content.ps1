...
function Get-PlainTextFromMarkdownLine {
  param([string]$Line)
  ...
  $text = [regex]::Replace($text, '<image[^>]*>', '')
  $text = [regex]::Replace($text, '\[([^\]]+)\]\(([^)]+)\)', '$1')
  $text = [regex]::Replace($text, '`([^`]+)`', '$1')
  $text = [regex]::Replace($text, 'https?://\S+', '')
  $text = $text -replace '原文链接[:：]?', ''
  $text = $text -replace '\*\*(.+?)\*\*', '$1'
  $text = $text -replace '\*(.+?)\*', '$1'
...
function Get-AutoSummary {
  ...
  $normalizedTitle = Normalize-TextForSummaryCompare -Text $FallbackTitle

  foreach ($line in $lines) {
    $plain = Get-PlainTextFromMarkdownLine -Line $line
    if ([string]::IsNullOrWhiteSpace($plain)) { continue }
    if ((Normalize-TextForSummaryCompare -Text $plain) -eq $normalizedTitle) { continue }
    if (Test-GenericIntroLine -Text $plain) { continue }
    if ($plain -match '^(配图|image|img)$') { continue }
    if ($plain -match '^[-_]{3,}$') { continue }
    $parts.Add($plain)
    if ($parts.Count -ge 4) { break }
  }

  $seed = ""
  if ($parts.Count -ge 2 -and ($parts[0].Length -lt 12 -or $parts[0] -match '[:：]\s*$')) {
    $seed = "$($parts[0]) $($parts[1])"
  } elseif ($parts.Count -ge 1) {
    $seed = "$($parts[0])"
  }

  $joined = (($seed -replace '\s+', ' ').Trim())
  if ([string]::IsNullOrWhiteSpace($joined)) {
    $joined = ("$FallbackTitle").Trim()
  }
  if ([string]::IsNullOrWhiteSpace($joined)) {
    return "基于最新内容自动生成的一句话摘要。"
  }

  $m = [regex]::Match($joined, '^(.{8,50}?(?:。|！|？|!|\?|；|;|\.{3,}|…{1,}))')
  if ($m.Success) {
    return $m.Groups[1].Value.Trim()
  }

  if ($joined.Length -gt 50) {
    $joined = $joined.Substring(0, 50).TrimEnd()
...
function Should-AutoGenerateDescription {
  param(
    [string]$Description,
    [string]$Title,
    [bool]$AutoRouting
  ...
  if ($placeholders -contains $text) { return $true }

  $normalizedDescription = Normalize-TextForSummaryCompare -Text $text
  $normalizedTitle = Normalize-TextForSummaryCompare -Text $Title
  if (-not [string]::IsNullOrWhiteSpace($normalizedTitle) -and $normalizedDescription -eq $normalizedTitle) {
    return $true
  }
  if ($AutoRouting -and (Test-GenericIntroLine -Text $text)) { return $true }
  if ($AutoRouting -and $text -match '[:：]\s*$') { return $true }
  if ($AutoRouting -and $text.Length -ge 48 -and $text -notmatch '[。！？!?\.;；…]$') { return $true }

  return ($AutoRouting -and -not [string]::IsNullOrWhiteSpace($normalizedDescription) -and -not [string]::IsNullOrWhiteSpace($normalizedTitle) -and (
      $normalizedTitle.StartsWith($normalizedDescription) -or
      $normalizedDescription.StartsWith($normalizedTitle)
    ))
}

function Normalize-TextForSummaryCompare {
  param([string]$Text)
...
function Test-GenericIntroLine {
  param([string]$Text)

  $value = "$Text".Trim()
  if ([string]::IsNullOrWhiteSpace($value)) { return $false }
  return (
    $value -match '^(hi|hello|哈喽|你好|大家好)[，,\s]*(我是|这里是)?\s*(kunki|坤琦|坤琪|王坤馨)?[。.!！\s]*$' -or
    $value -match '^一个普通大学生[，,].*AI浪潮.*象牙塔外的答案[。.!！\s]*$'
  )
}
...
$rawDescription = if ($meta.PSObject.Properties["description"]) { "$($meta.description)" } else { "" }
$description = "$rawDescription".Trim()
$autoRouting = ($meta.PSObject.Properties["autoRouting"] -and $meta.autoRouting -eq $true)
if (Should-AutoGenerateDescription -Description $description -Title $title -AutoRouting $autoRouting) {
  $description = Get-AutoSummary -Markdown $markdown -FallbackTitle $title
  if (-not $DryRun) {
    $meta.description = $description
    Save-JsonFile -Path $metadataPath -Data $meta
  }
  Write-Info "已自动生成摘要：$description"
}

if ([string]::IsNullOrWhiteSpace($description)) {
  throw "无法生成摘要，请检查 content.md 内容是否为空：$contentPath"
}
